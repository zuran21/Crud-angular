...
export class AgregarCategoriaComponent implements OnInit {
  ...
  formSubmit (){
    if (this.categoria.titulo.trim() == '' || this.categoria.titulo == null){
      this.snack.open("♦♦ El Titulo es nesesario !!♦♦", '',{duration:3000})
      return;
    }

    if(this.categoria.descripcion.trim() == '' || this.categoria.descripcion == null){
      this.snack.open("♣♣ El segmento es requerido ♠♠", '',{duration:3000})
    }

    this.catservice.agregarCategoria(this.categoria).subscribe(
      (dato:any) => { 
        Swal.fire("La Categoria fue agregada con exito Pokemon", "success");
